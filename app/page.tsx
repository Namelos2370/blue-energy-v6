...
import ChatBot from '../components/ChatBot'; // <--- NOUVEAU CHATBOT
...
export default function Home() {
  ...
  const [siteViews, setSiteViews] = useState(0); // État pour les vues
  ...
  useEffect(() => {
    ...
    // 2. Gestion Compteur de Vues (Simulation locale persistante)
    const savedViews = localStorage.getItem('be-views');
    let currentViews = savedViews ? parseInt(savedViews) : 5420; // On commence à 5420 pour faire pro
    // On incrémente la vue à chaque chargement de page (visite)
    currentViews += 1;
  ...
  return (
    <div className="min-h-screen bg-white text-[#0A1128] font-sans selection:bg-blue-100 overflow-x-hidden pt-8"> 
      {/* pt-8 ajouté pour compenser la TopBar fixe */}
